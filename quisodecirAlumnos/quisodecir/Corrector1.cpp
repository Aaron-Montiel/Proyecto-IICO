	...
	int &   iNumSugeridas)                      //Numero de elementos en la lista
{
	...
	// Cambiar a memoria
	char (*szSugerencias)[TAMTOKEN] = new char[MAX_SUG][TAMTOKEN];
	int num = 0;
	char palabra[TAMTOKEN];
	...
	int k = 1;
	for (int i = 1; i < num; ++i) {
		if (strcmp(szSugerencias[i], szSugerencias[k - 1]) != 0) {
			if (k != i) {
				strcpy_s(szSugerencias[k], TAMTOKEN, szSugerencias[i]);
			}
			++k;
		}
	}
